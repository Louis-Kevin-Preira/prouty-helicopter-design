...
plotted, so the relations are coded exactly rather than digitised:
    ...
    altitude, dT (nn,) --> rho, V_son, density_ratio, T_air (nn,)
...
class AtmosphereComp(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('dT', val=np.zeros(nn), units='degR',
                       desc='temperature offset from the standard day')

        self.add_output('rho', val=np.full(nn, RHO_0), units='slug/ft**3', desc='air density')
        self.add_output('V_son', val=np.full(nn, A_0), units='ft/s', desc='speed of sound')
        self.add_output('density_ratio', val=np.ones(nn), desc='rho / rho_0')
        self.add_output('T_air', val=np.full(nn, T_0), units='degR', desc='air temperature')

        self.declare_partials(['rho', 'density_ratio', 'V_son', 'T_air'],
                              ['altitude', 'dT'], rows=ar, cols=ar)

    def _state(self, inputs):
        h, dT = inputs['altitude'], inputs['dT']
        if np.any(np.real(h) > H_TROP):
            warnings.warn(f'altitude {np.max(np.real(h)):.0f} ft is above the {H_TROP:.0f} ft '
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials['T_air', 'altitude'] = dT_dh * np.ones_like(T)
        partials['T_air', 'dT'] = np.ones_like(T)
