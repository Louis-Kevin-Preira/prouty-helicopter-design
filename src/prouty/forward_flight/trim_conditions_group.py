...
class TrimConditionsGroup(om.Group):
    ...
    def setup(self):
        ...
        # A steep climb at low speed needs a very nose-down fuselage: 3,000
        ...
        # section 5.
        # It must be ON for the closed form too, for a different reason
        # (Chapter 4 diagnostic D): with it off the Newton unknowns include
        # every explicit output of the trim, and those are unscaled -- hp and
        # H_M start with residuals of 3e5 against 0.1 for alpha_F and
        # C_T/sigma. The step and the line search then follow the horsepower,
        # convergence turns linear, and in climb the iteration is walked onto
        # the alpha_F bound and settles there on a false root: five times the
        # weight in thrust and negative rotor power, with both trim residuals
        # at zero. With subsystem solves on, climb at zero rate reproduces
        # level flight exactly and the iteration count drops from 13 to 3.
        newton = self.nonlinear_solver = om.NewtonSolver(
            solve_subsystems=True, max_sub_solves=100)
        # with subsystem solves on, the residual norm is dominated by the
        # unscaled explicit outputs, so the tolerance has to be tighter to
        # leave the trim residuals themselves below 1e-12
